...
#define SOUND_DEVICE_VIRT_DEEP_PLAYBACK         101   // Deep playback virtual device (calliope_1)
...
// Capture devices
#define SOUND_DEVICE_VIRT_PRIMARY_CAPTURE       120      // primary capture virtual device (calliope_20)
#define SOUND_DEVICE_VIRT_LOWLATENCY_CAPTURE    121      // low-latency capture virtual device (calliope_21)
#define SOUND_DEVICE_VIRT_MMAP_CAPTURE          122      // MMAP capture virtual device (calliope_22)

#define SOUND_DEVICE_VIRT_VOICE_REF_TX          131
#define SOUND_DEVICE_VIRT_VTX_CAPTURE           123      // Voice call TX virtual device (calliope_23)
#define SOUND_DEVICE_CALL_RECORD                125      // WDMA for Call Recording (calliope_25)
#define SOUND_DEVICE_VIRT_FM_RECORD             124      // WDMA for FM Radio Recording (calliope_24)
#define SOUND_DEVICE_VIRT_SPKAMP_REFERENCE      128      // SmartPA IV-sense reference capture (calliope_28)
                                                          // NOTE: confirm this DAI link/device number exists
                                                          // on this kernel's ASoC card before relying on it -
                                                          // it is not auto-created by this #define.
...
#define OFFLOAD_PLAYBACK_DEVICE         SOUND_DEVICE_ABOX_RDMA2  // RDMA1 absent in this kernel
...
#define MAX_LOOP_PCM_NODES  2

struct loop_pcm_node {
    unsigned int      card;
    unsigned int      device;
    unsigned int      flags;     /* PCM_IN (capture) or PCM_OUT (playback) */
    struct pcm_config config;
    const char       *name;      /* used in log messages */
    bool              required;  /* false = silently skip if node not present */
};

static const struct loop_pcm_node speaker_loop_pcm_nodes[MAX_LOOP_PCM_NODES] = {
    {
        .card     = SOUND_CARD0,
        .device   = SOUND_DEVICE_VIRT_SPKAMP_REFERENCE,
        .flags    = PCM_IN,
        .config   = {
            .channels        = SPKAMP_REFERENCE_CHANNELS,
            .rate            = SPKAMP_REFERENCE_SAMPLING_RATE,
            ...
            .period_count    = SPKAMP_REFERENCE_PERIOD_COUNT,
            .format          = SPKAMP_REFERENCE_FORMAT,
            .start_threshold = 0,           /* start immediately on pcm_start */
            .stop_threshold  = ULONG_MAX,
        },
        .name     = "SPKAMP Reference",
        .required = false,
    },
    {
        .card     = SOUND_CARD0,
        .device   = MUTE_PLAYBACK_DEVICE,
        .flags    = PCM_OUT,
        .config   = {
            .channels        = MUTE_PLAYBACK_CHANNELS,
            .rate            = MUTE_PLAYBACK_SAMPLING_RATE,
...
// PCM Configurations for Voice Reference TX Capture Stream
#define VOICE_REF_TX_CARD                SOUND_CARD0
#define VOICE_REF_TX_DEVICE              SOUND_DEVICE_VIRT_VOICE_REF_TX

#define VOICE_REF_TX_CHANNELS            DEFAULT_VOICE_CHANNELS
#define VOICE_REF_TX_SAMPLING_RATE       DEFAULT_VOICE_SAMPLING_RATE
#define VOICE_REF_TX_PERIOD_SIZE         480
#define VOICE_REF_TX_PERIOD_COUNT        4
#define VOICE_REF_TX_FORMAT              DEFAULT_VOICE_FORMAT
#define VOICE_REF_TX_START               VOICE_REF_TX_PERIOD_SIZE
#define VOICE_REF_TX_STOP                ULONG_MAX

struct pcm_config pcm_config_voice_ref_tx = {
    .channels        = VOICE_REF_TX_CHANNELS,
    .rate            = VOICE_REF_TX_SAMPLING_RATE,
    .period_size     = VOICE_REF_TX_PERIOD_SIZE,
    .period_count    = VOICE_REF_TX_PERIOD_COUNT,
    .format          = VOICE_REF_TX_FORMAT,
    .start_threshold = VOICE_REF_TX_START,
    .stop_threshold  = VOICE_REF_TX_STOP,
};
